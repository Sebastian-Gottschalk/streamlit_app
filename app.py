import streamlit as st
import pandas as pd
import matplotlib.pyplot as plt
import plotly.express as px
import plotly.graph_objects as go
from copy import deepcopy
from tools.buttons import *

path = "./data/mpg.csv"

@st.cache_data
def load_data(path):
    df = pd.read_csv(path)
    return df

st.write(st.session_state)

if "tab1" not in st.session_state:
    st.session_state.tab1 = False


if "tab2" not in st.session_state:
    st.session_state.tab2 = False



mpg_df_raw = load_data(path)
mpg_df = deepcopy(mpg_df_raw)

st.title("Introduction to Streamlit")
st.header("MPG Data Exploration")

col1,col2,col3,col4 = st.columns(4)

col1.button("Tab 1", on_click=tab1click)
col2.button("Tab 2", on_click=tab2click)
col3.button("Tab 3")
col4.button("Tab 4")

if st.session_state.tab1: 
    # st.table(mpg_df)
    if st.checkbox("Show Dataframe"):
        st.subheader("This is my dataset:")
        st.dataframe(mpg_df)

    left_column,middle_column, middle_right_column, right_column = st.columns([2,1.5,2,1])
    # 3 + 1 + 1 = 5
    # first column: 3/5 of the total space

    years = ["All"] + sorted(pd.unique(mpg_df["year"]))
    year = left_column.selectbox("Choose a year", years)

    show_means = middle_column.radio("Show Class Means", ["Yes","No"])

    # model = middle_right_column.multiselect("Select Models", pd.unique(mpg_df["model"]))
    model = middle_right_column.text_input("Select Model")


    if year == "All":
        reduced_df = mpg_df
    else:
        reduced_df = mpg_df[mpg_df["year"] == year]

    if model in pd.unique(reduced_df["model"]):
        reduced_df = reduced_df[reduced_df["model"] == model]

    means = reduced_df.groupby("class").mean(numeric_only=True)

    plot_types = ["Matplotlib", "Plotly"]
    plot_type = right_column.radio("Choose Plot Type", plot_types)


    m_fig, ax = plt.subplots(figsize = (10,8))
    ax.scatter(reduced_df["displ"],reduced_df["hwy"], alpha = 0.7)
    ax.set_title("Engine Size vs Highway Fuel Mileage")
    ax.set_xlabel("Displacement (Liters)")
    ax.set_ylabel("MPG")
    if show_means == "Yes":
        ax.scatter(means["displ"], means["hwy"], alpha = 0.7, color= "red", label= "Class Means")

    # st.pyplot(m_fig)


    p_fig = px.scatter(reduced_df, x="displ", y="hwy", opacity = 0.5,
                    range_x = [1,8], range_y = [10,50],
                    width = 750, height = 600,
                    labels = {"displ" : "Displacement (Liters)",
                                "hwy" : "MPG"},
                        title = "Engine Size vs Highway Fuel Mileage")
    p_fig.update_layout(title_font_size = 22)
    if show_means == "Yes":
        p_fig.add_trace(go.Scatter(x=means["displ"], y=means["hwy"], mode = "markers"))
        p_fig.update_layout(showlegend = False)


    # st.plotly_chart(p_fig)

    if plot_type == "Matplotlib":
        st.pyplot(m_fig)
    else:
        st.plotly_chart(p_fig)

if st.session_state.tab2:
    url = "https://archive.ics.uci.edu/ml/datasets/auto+mpg"
    st.write("Data Source", url)

    st.subheader("Streamlit Map")
    ds_geo = px.data.carshare()

    ds_geo["lat"] = ds_geo["centroid_lat"]
    ds_geo["lon"] = ds_geo["centroid_lon"]

    st.map(ds_geo)